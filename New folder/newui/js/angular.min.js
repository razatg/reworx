...
return match;});message=message+'\nhttp://errors.angularjs.org/1.3.13/'+(module?module+'/':'')+code;for(i=2;i<arguments.length;i++){message=message+(i==2?'?':'&')+'p'+(i-2)+'='+encodeURIComponent(toDebugString(arguments[i]));}
...
function trimEmptyHash(url){if(url)
...
if(hasApply){return function(){var args=[];forEach(arguments,function(arg){args.push(formatError(arg));});return 1;};}
